...
<head>
    ...
    <title>Kelola Aset Hardware - KOMINFO</title>
                                ...
                                <div class="data-tables datatable-dark">
                                    <table id="dataTable3" class="display" style="width:100%">
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($data_hardware as $aset)
                                            <tr>
                                                <td>{{$aset->id}}</d>
                                                <td>{{$aset->category_id}}</d>
                                                <td>{{$aset->nama_item}}</td>
                                                <td><img src="{{$aset->gambar}}" alt="" width="50%"></td>
                                                <td>{{$aset->keterangan}}</td>
                                                <td>{{$aset->pemegang}}</td>
                                                <td></td>
                                                <form method="POST" action="/hardwareadm {{ $aset->id}}">
                                                    @method('delete')
                                                    @csrf
                                                    <td>
                                                        <button class="btn btn-danger" onclick="return confirm('Yakin Mau Menghapus Barang ?')">Hapus</button></a>

                                                    </td>
                                                </form>
                                                <td>
                                                    <button class="btn btn-danger" data-toggle="modal" data-target="#editModal-{{$aset->id}}">Edit</button></a>
                                                </td>
                                            </tr>
                                            @endforeach
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="/hardwareadm" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label>Nama Aset</label>
                            <input name="nama_item" type="text" class="form-control" required autofocus>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input name="keterangan" type="text-area" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label>Pemegang</label>
                            <input name="pemegang" type="text" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label>Gambar (Harus PNG)</label>
    ...
    <div id="editModal-{{$data->id}}" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="/hardwareadm {{$data->id}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label>Nama Aset</label>
                            <input name="nama_item" type="text" class="form-control" required autofocus value="{{$data->nama_item}}">
                        </div>
                        <div class="form-group">
                            <label>Kategori</label>
                            <select class="form-select" name="category_id">
                        ...
                        <div class="form-group">
                            <label>Keterangan</label>
                            <input name="keterangan" type="text-area" class="form-control" required value="{{$data->keterangan}}">
                        </div>
                        <div class="form-group">
                            <label>Pemegang</label>
                            <input name="pemegang" type="text" class="form-control" required required value="{{$data->pemegang}}">
                        </div>
                        <div class="form-group">
                            <label>Gambar (Harus PNG)</label>
                            <input name="gambar" type="file" class="form-control">
